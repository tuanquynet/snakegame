...
function Snake(x, y, color, game, id) {
...
Snake.prototype.update = function () {
	...
	newHead = this.adjustAgainstWall(newHead);
...
Snake.prototype.adjustAgainstWall = function (newHead) {
	if (newHead.x() >= config.XSIZE) {
		newHead.setX(0);
	} else if (newHead.x() < 0) {
		newHead.setX(config.XSIZE - 1);
	}
	if (newHead.y() >= config.YSIZE) {
		newHead.setY(0);
	} else if (newHead.y() < 0) {
		newHead.setY(config.YSIZE - 1);
	}
	return newHead;
};
...
Snake.prototype.doTurn = function (dir) {
	...
	var isTurned = (dir === 'l' && (this.head.x() <= second.x()));
	isTurned = isTurned || (dir === 'u' && (this.head.y() <= second.y()));
	isTurned = isTurned || (dir === 'r' && (this.head.x() >= second.x()));
	isTurned = isTurned || (dir === 'd' && (this.head.y() >= second.y()));
...
Snake.prototype.compensate = function (coords, tick) {
	...
	for (i = 0; i < this.game.curTick() - tick; i++) {
		this.update();
	}
};
...
Snake.prototype.increaseSize = function (num) {
	var lastPart;
	var i = 0;
	while (i < num) {
		lastPart = this.body[0];
		lastPart = new Gridbox(lastPart.x(), lastPart.y());
		this.body.unshift(lastPart);
		i++;
	}
};
...
/*Extend hasCollided
collide wall: go through the wall to other side
snake can go through together
*/
